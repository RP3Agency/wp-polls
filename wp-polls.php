...
Version: 2.63a
Authors: Lester 'GaMerZ' Chan, Tracy Rotton
...
function display_pollvote($poll_id, $display_loading = true) {
	...
	$poll_question = $wpdb->get_row("SELECT pollq_id, pollq_question, pollq_followup, pollq_totalvotes, pollq_timestamp, pollq_expiry, pollq_multiple, pollq_totalvoters FROM $wpdb->pollsq WHERE pollq_id = $poll_id LIMIT 1");
	// Poll Question Variables
	$poll_question_text = stripslashes($poll_question->pollq_question);
	$poll_followup_line = stripslashes($poll_question->pollq_followup);
	...
	$template_question = str_replace("%POLL_FOLLOWUP%", $poll_followup_line, $template_question);
...
function display_pollresult($poll_id, $user_voted = '', $display_loading = true) {
	...
	// Get Poll Question Data
	$poll_question = $wpdb->get_row("SELECT pollq_id, pollq_question, pollq_followup, pollq_totalvotes, pollq_active, pollq_timestamp, pollq_expiry, pollq_multiple, pollq_totalvoters FROM $wpdb->pollsq WHERE pollq_id = $poll_id LIMIT 1");
	// Poll Question Variables
	$poll_question_text = stripslashes($poll_question->pollq_question);
	$poll_followup_line = stripslashes($poll_question->pollq_followup);
	$poll_question_id = intval($poll_question->pollq_id);
	...
	if($poll_question && $poll_answers) {
		...
		$template_footer = str_replace("%POLL_FOLLOWUP%", $poll_followup_line, $template_footer);
		if($poll_multiple_ans > 0) {
			$template_footer = str_replace("%POLL_MULTIPLE_ANS_MAX%", $poll_multiple_ans, $template_footer);
		} else {
			$template_footer = str_replace("%POLL_MULTIPLE_ANS_MAX%", '1', $template_footer);
...
function polls_archive() {
	...
	foreach($polls_questions as $polls_question) {
		...
		$template_question = str_replace("%POLL_FOLLOWUP%", $polls_question['followup'], $template_question);
...
function create_poll_table() {
	...
	$create_table['pollsq'] = "CREATE TABLE $wpdb->pollsq (".
									...
									"pollq_followup varchar(256) character set utf8 default '',".
	...
	// Database Upgrade For WP-Polls 2.63a
	maybe_add_column($wpdb->pollsq, 'pollq_followup', "ALTER TABLE $wpdb->pollsq ADD pollq_followup varchar(265) NOT NULL default '' AFTER pollq_question;");
