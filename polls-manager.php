...
if(!empty($_POST['do'])) {
	...
	switch($_POST['do']) {
		...
		case __('Edit Poll', 'wp-polls'):
			...
			// Poll Follow Up Line
			$pollq_followup = addslashes(trim($_POST['pollq_followup']));
			// Poll Active
			...
			$edit_poll_question = $wpdb->query("UPDATE $wpdb->pollsq SET pollq_question = '$pollq_question', pollq_followup = '$pollq_followup', pollq_totalvotes = $pollq_totalvotes, pollq_expiry = '$pollq_expiry', pollq_active = $pollq_active, pollq_multiple = $pollq_multiple, pollq_totalvoters = $pollq_totalvoters $timestamp_sql WHERE pollq_id = $pollq_id");
...
switch($mode) {
	...
	case 'edit':
		...
		$poll_question = $wpdb->get_row("SELECT pollq_question, pollq_followup, pollq_timestamp, pollq_totalvotes, pollq_active, pollq_expiry, pollq_multiple, pollq_totalvoters FROM $wpdb->pollsq WHERE pollq_id = $poll_id");
...
?>
		...
		<div class="wrap">
			...
			<!-- Poll Follow Up Line -->
			<h3><?php _e('Poll Follow Up Line', 'wp-polls'); ?></h3>
			<table class="form-table">
				<tr>
					<th width="20%" scope="row" valign="top"><?php _e('Follow Up Line', 'wp-polls'); ?></th>
					<td width="80%"><textarea name="pollq_followup" cols="70" rows="2"><?php echo htmlspecialchars($poll_followup_line); ?></textarea><br />(256 Characters Max)</td>
				</tr>
			</table>
